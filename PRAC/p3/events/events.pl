...
satVariable( md(M,D) ) :- moderator(M), day(D), event(E),
			em(E,M), ed(E,D).
% satVariable( md(M,D) ) :-  event(E), eventModerators(E, ModList),
%			moderator(M), member(M, ModList), em(E,M),
%			day(D), eventDays(E,DayList),
%			member(D,DayList), ed(E,D).
...
eachEventExactlyOneDay :-
	event(E),
	eventDays(E,DayList),
	findall(ed(E,D), (day(D), member(D,DayList)) , Lits),
	exactly(1,Lits),
	fail.
eachEventExactlyOneDay.
...
exactlyOneModeratorPerEvent :-
	event(E),
	eventModerators(E,ModList),
	findall(em(E,M), (moderator(M), member(M, ModList)), Lits),
	exactly(1,Lits),
	fail.
exactlyOneModeratorPerEvent.
